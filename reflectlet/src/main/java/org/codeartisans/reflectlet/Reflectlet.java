        ...
        extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        ...
        data.put( "SSL Cipher", request.getAttribute( "javax.servlet.request.cipher_suite" ) );
        data.put( "SSL Key Size", request.getAttribute( "javax.servlet.request.key_size" ) );
        data.put( "SSL SessionID", request.getAttribute( "javax.servlet.request.ssl_session" ) );
        data.put( "ClientCertificate", getClientX509CertificateInfo( request ) );
        table( writer, data );
        data.clear();

        h2( writer, "Request attributes" );
        @SuppressWarnings( "unchecked" )
        Enumeration<String> attributeNames = request.getAttributeNames();
        while ( attributeNames.hasMoreElements() ) {
            String eachAttributeName = attributeNames.nextElement();
            data.put( eachAttributeName, request.getAttribute( eachAttributeName ) );
        }
        table( writer, data );
        data.clear();
